...
DEFINE_BLITTER(BlitTrans)
{
public:
	inline explicit BlitTrans() noexcept
	{}

	virtual ~BlitTrans() override final = default;

	virtual void Blit_Copy(void* dst, byte * src, int len, int zval, WORD * zbuf, WORD * abuf, int alvl, int warp) override final
	{
		for (int i = 0; i < len; ++i)
		{
			if (const auto srcv = reinterpret_cast<T*>(src)[i])
				reinterpret_cast<T*>(dst)[i] = srcv;
		}
	}
